...
<link rel="stylesheet" type="text/css" href="css/examplestwo.css"
	media="screen" />
</head>
...
<body>
				...
				<ul class="nav justify-content-end"
					...
					<li class="nav-item"><c:choose>
							<c:when test="${empty LoginOK}">
								<a class="nav-link" href="<c:url value="/member/member_login"/>"
									id="memberlogin" style="color: rgb(92, 41, 7);">會員登出</a>
							</c:when>
							<c:otherwise>
								<a class="nav-link"
									href="<c:url value="/member/member_logout"/>" id="memberlogout"
									style="color: rgb(92, 41, 7);">會員登出</a>
							</c:otherwise>
						</c:choose></li>
					...
					<li class="nav-item"><a class="nav-link"
						...
						style="color: rgb(92, 41, 7);"> <i
							class="fas fa-shopping-cart"><span style="color: #FF5964"
								id="shoppingCartItem">${shoppingCart}</span> <!-- 加入購物車的商品數量 --></i>
					</a></li>
				</ul>
			</div>
		</div>
	...
	<div class="col-7 textartccal">
		...
		<p>
			<span id="acttime">${news.actTime}</span>
		</p>
		<div class="textartccalimg">
			<img
				src="${pageContext.servletContext.contextPath}/news/RetrieveNewsImg?id=${news.newsId}"
				id="newsimg">
		</div>
		<div style="text-align:left" id="contextstr">
			 ${content}
...
//            	 if(i>0 && src.charAt(i) == '【' &&  src.charAt(i-1) =='▶' ) {  用不到，前面太多空白
//         		 out += src.charAt(i);
//         		 continue;
//         	 }		
